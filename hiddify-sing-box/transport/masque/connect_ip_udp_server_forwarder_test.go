...
func TestConnectIPUDPForwarderSendICMPPortUnreachable(t *testing.T) {
	t.Parallel()
	...
	f := &connectIPTCPForwarder{conn: rec}
	...
	if _, _, ok := parseICMPPortUnreachablePeer(rec.written[0]); !ok {
		t.Fatalf("not icmp port unreachable pkt_len=%d", len(rec.written[0]))
	}
}

func TestConnectIPUDPForwarderICMPPortUnreachableOnClosedPort(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
		t.Skip("connected UDP ICMP port-unreachable is unreliable on Windows")
	}
	// Closed UDP socket: kernel should return ICMP port-unreachable on write/read.
	...
	peer, p, ok := parseICMPPortUnreachablePeer(icmpReply)
