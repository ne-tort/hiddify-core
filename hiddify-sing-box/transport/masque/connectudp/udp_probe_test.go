...
func TestUDPProbeAnalyzeNoLoss(t *testing.T) {
	...
	for i := range sent {
		sink.Record(connectudp.BuildProbePayload(uint64(i), runID, payloadLen))
	}
...
func TestSequencedSinkReset(t *testing.T) {
	...
	sink := connectudp.NewSequencedSink(runA)
	for i := range 50 {
		sink.Record(connectudp.BuildProbePayload(uint64(i), runA, payloadLen))
	}
	sink.Reset(runB)
	for i := range 40 {
		sink.Record(connectudp.BuildProbePayload(uint64(i), runB, payloadLen))
	}
