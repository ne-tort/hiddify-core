...
//
// Smoke: H3/H2 CoreSession echo, split-payload (H3 skip Windows — helpers_test.go), forbidden,
// H2/H3 soft ICMP port-unreachable (Linux; Windows skip).
// Interop: masque-go Client.DialAddr → in-proc relay (Wave 3, h3_masque_go_interop_test.go).
// Localize gates: echo KPI, synth localize (H2/H3 prod profile), docker paced / soft-ICMP shape,
