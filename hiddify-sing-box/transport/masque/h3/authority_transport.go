...
// NewAuthorityHTTP3Transport builds an isolated HTTP/3 stack for connect_authority only.
// Intentionally does not use masque connect_stream QUIC tuning, datagram SETTINGS, or warp extras.
func NewAuthorityHTTP3Transport(cfg AuthorityClientConfig) *http3.Transport {
	tlsCfg := cfg.TLS
	if tlsCfg == nil {
