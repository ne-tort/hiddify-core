...
// DialH3Params configures one HTTP/3 CONNECT-by-authority dial.
type DialH3Params struct {
	Tag              string
	MasqueHost       string
	MasquePort       uint16
...
// DialH3 opens CONNECT-by-authority. Production uses connectauthority.Client via masque coreSession.
func DialH3(ctx context.Context, p DialH3Params) (*Conn, error) {
	if p.RoundTripper != nil {
		return dialH3RoundTrip(ctx, p)
	}
...
func dialH3RoundTrip(ctx context.Context, p DialH3Params) (*Conn, error) {
	connectURL, err := ExpandConnectURL(p.TemplateConnect, p.TargetHost, p.TargetPort)
	if err != nil {
		return nil, errors.Join(ErrConnectAuthorityFailed, err)
	}
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		if strings.TrimSpace(os.Getenv("MASQUE_TRACE_TCP")) == "1" {
			log.Printf("connectauthority roundtrip url=%s host=%s pipe_fallback=%t", connectURL.String(), connectHost, allowPipe)
		}
		...
		conn, _, derr := dialConnFromResponse(ctx, resp, pw, p.TargetHost, p.TargetPort, allowPipe)
		if derr != nil {
			lastErr = derr
