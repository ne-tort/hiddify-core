...
func TestGATEConnectUDPH3SynthProdDownloadFountain(t *testing.T) {
	...
	if _, err := pkt.WriteTo(prime, fountainAddr); err != nil {
		t.Fatalf("prime WriteTo: %v", err)
	}
	time.Sleep(50 * time.Millisecond)
	bytes, mbps, err := benchConnectUDPPacketReceiveOnly(pkt, dur, connectudp.DefaultBenchUDPPayloadLen)
	if err != nil {
...
func TestLocalizeConnectUDPH3DownloadPipelineDepth(t *testing.T) {
	...
	for _, depth := range []int{1, 8, 64, 128, 256, 0} {
		...
		_, mbps, err := benchConnectUDPPacketDownloadPipelined(pkt, echoAddr, dur, payloadLen, depth)
		if err != nil {
...
func benchConnectUDPPacketReceiveOnly(pkt net.PacketConn, duration time.Duration, payloadLen int) (int64, float64, error) {
	buf := make([]byte, payloadLen+64)
	deadline := time.Now().Add(duration)
	var received int64
	for time.Now().Before(deadline) {
...
func TestLocalizeConnectUDPH3DownloadFountain(t *testing.T) {
	...
	time.Sleep(50 * time.Millisecond)
	_, mbps, err := benchConnectUDPPacketReceiveOnly(pkt, dur, connectudp.DefaultBenchUDPPayloadLen)
	if err != nil {
...
func TestLocalizeConnectUDPH3EchoDuplexGap(t *testing.T) {
	...
	prime := make([]byte, connectudp.DefaultBenchUDPPayloadLen)
	_ = fountainPkt.SetWriteDeadline(time.Now().Add(2 * time.Second))
	if _, err := fountainPkt.WriteTo(prime, fountainAddr); err != nil {
		t.Fatalf("fountain prime: %v", err)
	}
	time.Sleep(50 * time.Millisecond)
	_, fountainMbps, err := benchConnectUDPPacketReceiveOnly(fountainPkt, dur, connectudp.DefaultBenchUDPPayloadLen)
	_ = fountainPkt.Close()
	if err != nil {
	...
	_, echoMbps, err := benchConnectUDPPacketDownloadViaEcho(echoPkt, echoAddr, dur, connectudp.DefaultBenchUDPPayloadLen)
	if err != nil {
...
func TestLocalizeConnectUDPH3Pipeline1ProdShape(t *testing.T) {
	...
	defer func() { _ = pkt.Close() }()
	_, mbps, err := benchConnectUDPPacketDownloadPipelined(pkt, echoAddr, dur, connectudp.DefaultBenchUDPPayloadLen, 1)
	if err != nil {
...
func TestLocalizeConnectUDPH2EchoPipeline256VsUnlimited(t *testing.T) {
	...
	defer func() { _ = pkt.Close() }()
	payloadLen := connectudp.DefaultBenchUDPPayloadLen
	_, pipeline256, err := benchConnectUDPPacketDownloadPipelined(pkt, echoAddr, dur, payloadLen, 256)
	if err != nil {
		t.Fatalf("pipeline256: %v", err)
	}
	_, unlimited, err := benchConnectUDPPacketDownloadPipelined(pkt, echoAddr, dur, payloadLen, 0)
	if err != nil {
...
func TestLocalizeConnectUDPH2DownloadFountainMaxCapsule(t *testing.T) {
	...
	time.Sleep(50 * time.Millisecond)
	_, mbps, err := benchConnectUDPPacketReceiveOnly(pkt, dur, maxPayload)
	if err != nil {
...
func TestLocalizeConnectUDPH2DownloadFountainPayloadScaling(t *testing.T) {
	...
	runFountain := func(payloadLen int) float64 {
		...
		time.Sleep(50 * time.Millisecond)
		_, mbps, err := benchConnectUDPPacketReceiveOnly(pkt, dur, payloadLen)
		if err != nil {
...
func TestLocalizeConnectUDPH2DownloadFountainDirectDialVsListenPacket(t *testing.T) {
	...
	time.Sleep(50 * time.Millisecond)
	_, listenMbps, err := benchConnectUDPPacketReceiveOnly(pkt, dur, connectudp.DefaultBenchUDPPayloadLen)
	_ = pkt.Close()
	_ = session.Close()
	if err != nil {
