...
func runH3ProdStackFakeIperfDownloadMbps(t *testing.T, proxyPort int, targetPort uint16, duration time.Duration) (int64, float64, error) {
	...
	return masque.InttestMeasureTCPDownloadCopyMbps(conn, duration)
}
...
func TestLaunchMasqueStackH3WriteToOnlyIperfBanner(t *testing.T) {
	...
	go func() {
		n, _, err := masque.InttestMeasureTCPDownloadWriteToMbps(conn, h3DockerSynthDownloadBench)
...
func TestLocalizeDockerH335msIperfReverseWriteTo(t *testing.T) {
	floorMbps := masque.InttestConnectStreamDocker35msSeqDownFloorMbps()
	targetPort := masque.InttestStartH2FakeIperfStreamingDownloadTarget(t)
	proxyPort := startLaunchMasqueStackH3ConnectStreamServer(t)
	socksPort := masque.InttestStartH3ConnectStreamSocksRouter(t, proxyPort)
	conn := masque.InttestSocksTCPDial(t, socksPort, targetPort)
	conn = masque.InttestWrapBenchWindowedBidiLinkH3Prod(conn)
	if err := conn.SetDeadline(time.Now().Add(masque.InttestConnectStreamSynthProdBenchDuration() + 8*time.Second)); err != nil {
		t.Fatalf("set deadline: %v", err)
	}
	banner := make([]byte, 8)
	if _, err := io.ReadFull(conn, banner); err != nil {
	...
	masque.InttestAssertLocalizeDocker35msSequentialLeg(t, "tcp_down WriteTo", mbps, floorMbps)
}
...
func TestLocalizeDockerH3StrictL256Ceiling35ms(t *testing.T) {
	...
	socksPort := masque.InttestStartH3ConnectStreamSocksRouter(t, proxyPort)
	conn := masque.InttestSocksTCPDial(t, socksPort, targetPort)
	conn = masque.InttestWrapBenchWindowedBidiLinkStrictH3L256(conn)
	if err := conn.SetDeadline(time.Now().Add(masque.InttestConnectStreamSynthProdBenchDuration() + 8*time.Second)); err != nil {
		t.Fatalf("set deadline: %v", err)
	}
	banner := make([]byte, 8)
	...
	masque.InttestAssertLocalizeStrictL256Ceiling35ms(t, "docker strict L256", mbps)
}
...
func TestLaunchMasqueStackH3ConcurrentControlDuringWriteTo(t *testing.T) {
	...
	socksPort := masque.InttestStartH3ConnectStreamSocksRouter(t, proxyPort)
	conn := masque.InttestSocksTCPDial(t, socksPort, targetPort)
	conn = masque.InttestWrapBenchWindowedBidiLinkStrictH3(conn)
	...
	go func() {
		n, _, err := masque.InttestMeasureTCPDownloadWriteToMbps(conn, masque.InttestH3HonestGateDuration())
		done <- result{n: n, err: err}
	}()
	...
	case r := <-done:
		...
		if r.n < masque.InttestH3HonestGateMinBytes() {
			t.Fatalf("concurrent control download short: %d want >= %d (H3 bidi FC stall)", r.n, masque.InttestH3HonestGateMinBytes())
		}
	case <-time.After(masque.InttestH3HonestGateDuration() + 5*time.Second):
...
func TestLaunchMasqueStackH3ConnectStreamDownloadKPI(t *testing.T) {
	...
	if mbps < masque.ExportConnectStreamSynthProdMinMbps {
		t.Fatalf("%s", masque.InttestSynthKPIDiagnostic("[H3-L0 prod stack]", "tcp_down WriteTo", mbps,
