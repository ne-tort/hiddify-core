...
// wake from client WriteTo) before relaying to onward TCP. Without strip, real iperf3 control
// sees garbage and closes ("control socket has closed unexpectedly").
...
type h2BootstrapUploadStripper struct {
	inner   io.ReadCloser
	skip    int // remaining all-zero bootstrap bytes to discard
	pending []byte
	scratch [512]byte
}
