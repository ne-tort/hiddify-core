...
func TestBidiTunnelConnWriteToDownloadActive(t *testing.T) {
	...
	paths := TunnelPaths{
		Download: &downloadPathAdapter{inner: pr},
		Upload:   NewUploadPath(pw),
	}
	inner := ConnFromTunnelPaths(context.Background(), paths, &net.TCPAddr{}, &net.TCPAddr{Port: 443})
	bidi, ok := inner.(*bidiTunnelConn)
	if !ok {
		t.Fatalf("expected *bidiTunnelConn, got %T", inner)
	}
...
func TestBidiTunnelConnReadFromUploadPath(t *testing.T) {
	...
	bidi, ok := conn.(*bidiTunnelConn)
	if !ok {
		t.Fatalf("expected *bidiTunnelConn, got %T", conn)
	}
...
func TestBidiTunnelConnUploadDrainsPendingDownload(t *testing.T) {
	...
	// iperf server banner arrives on download after upload starts (drain must consume it).
	go func() {
		time.Sleep(20 * time.Millisecond)
		_, _ = respW.Write([]byte("iperf3\r\n"))
	}()
