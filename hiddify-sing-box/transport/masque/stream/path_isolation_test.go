...
// TestStreamPathIsolation guards P-STREAM package tree from P-UDP/P-IP imports (W-STR-4 PR5).
func TestStreamPathIsolation(t *testing.T) {
	t.Parallel()
	...
	if len(violations) > 0 {
		t.Fatalf("stream path isolation violated:\n%s", strings.Join(violations, "\n"))
	}
}
