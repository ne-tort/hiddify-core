...
func TestH2TunnelConnRouteDuplexSkipsUploadDrain(t *testing.T) {
	...
	_, _ = conn.ReadFrom(bytes.NewReader([]byte("bulk")))
	select {
	case <-dl.readEntered:
...
func TestH2TunnelConnWriteToUnblocksDrainReadLock(t *testing.T) {
	...
	inner := ConnFromTunnelPaths(context.Background(), paths, &net.TCPAddr{}, &net.TCPAddr{Port: 9})
	conn := NewTunnelConn(inner)
	inner.(*bidiTunnelConn).maybeStartDownloadDrain()
	...
	go func() {
		...
		_, _ = conn.WriteTo(bytes.NewBuffer(nil))
		done <- struct{}{}
	}()
...
func TestH2TunnelConnWriteToConcurrentUploadDrain(t *testing.T) {
	...
	inner := ConnFromTunnelPaths(context.Background(), paths, &net.TCPAddr{}, &net.TCPAddr{Port: 5201})
	conn := NewTunnelConn(inner)
	conn.MarkConnectionCopyDuplex()
...
func TestH2TunnelConnWriteToStopsDrainBeforeRead(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		if atomic.LoadInt32(&inner.(*bidiTunnelConn).drainStopped) != 0 {
			sink.release()
			return
		}
