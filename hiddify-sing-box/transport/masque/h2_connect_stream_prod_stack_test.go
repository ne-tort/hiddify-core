...
func TestLaunchMasqueStackH2WriteToOnlyIperfBanner(t *testing.T) {
	...
	go func() {
		n, _, err := masque.ExportMeasureTCPDownloadWriteToMbps(conn, 2*time.Second)
...
func TestLaunchMasqueStackH2ConnectStreamDownloadKPI(t *testing.T) {
	...
	if mbps < masque.ExportConnectStreamSynthProdMinMbps {
		t.Fatalf("%s", masque.ExportSynthKPIDiagnostic("[H2-L0 prod stack]", "tcp_down WriteTo", mbps,
...
func TestLaunchMasqueStackH2ConnectStreamDownloadKPIWindowedNoPulse(t *testing.T) {
	...
	conn := masque.ExportSocksTCPDial(t, socksPort, targetPort)
	conn = masque.ExportWrapBenchWindowedBidiLinkH2Prod(conn)
	if err := conn.SetDeadline(time.Now().Add(12 * time.Second)); err != nil {
		t.Fatalf("set deadline: %v", err)
	}

	wt, ok := masque.ExportWriterTo(conn)
	if !ok {
		t.Fatal("SOCKS masque conn lacks io.WriterTo")
	}
	_ = wt
...
func TestLaunchMasqueStackH2ConnectStreamDownloadKPINetem(t *testing.T) {
	...
	if mbps <= masque.ExportConnectStreamVPSKPITargetDown {
		t.Fatalf("netem prod stack download: %.1f Mbit/s (want > %.0f KPI)", mbps, masque.ExportConnectStreamVPSKPITargetDown)
	}
}
...
func TestCMRouteWriterToBranchViaSocks(t *testing.T) {
	...
	_, _, _ = masque.ExportMeasureTCPDownloadWriteToMbps(conn, 500*time.Millisecond)
...
func TestLaunchMasqueStackH2ConcurrentControlDuringWriteTo(t *testing.T) {
	...
	go func() {
		n, _, err := masque.ExportMeasureTCPDownloadWriteToMbps(conn, h2HonestGateDuration)
		done <- result{n: n, err: err}
	}()
	...
	case r := <-done:
		...
		if r.n < h2HonestGateMinBytes {
			t.Fatalf("concurrent control download short: %d want >= %d (H2 bidi FC stall)", r.n, h2HonestGateMinBytes)
		}
	case <-time.After(h2HonestGateDuration + 5*time.Second):
