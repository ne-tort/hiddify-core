...
func (s *coreSession) tryHTTPFallbackSwitch(err error) bool {
	return session.TryHTTPFallbackSwitch(&s.CoreSession, s.overlaySwitchHost(), err)
}

func (s *coreSession) tryHTTPFallbackSwitchLockedAssumeMu(err error) bool {
	return session.TryHTTPFallbackSwitchLockedAssumeMu(&s.CoreSession, s.overlaySwitchHost(), err)
}
