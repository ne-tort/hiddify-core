...
// LIFE-5: bidi cleanup after upload→download→close (native packet plane, hybrid connect_stream, CM relay).
...
// or goroutine leaks on native H3 packet plane, hybrid connect_stream, and CM netstack paths.
func RunGATEConnectIPBidiCleanupAfterUploadDownload(t *testing.T) {
	...
	t.Run("HybridH3ConnectStream", runBidiCleanupHybridH3ConnectStream)
...
func runBidiCleanupHybridH3ConnectStream(t *testing.T) {
	t.Helper()
	uploadLn := masque.StartConnectIPNativeUploadSink(t)
	downLn := StartHybridConnectIPDownloadTarget(t)
	srv := NewHybridConnectIPH3Server(t)
	...
	beforeSess := goroutineCount()
	sess, err := (masque.CoreClientFactory{}).NewSession(ctx, HybridConnectStreamH3ClientOptions(srv.Port()))
	if err != nil {
		t.Fatalf("session: %v", err)
	}
	if _, err := sess.OpenIPSession(ctx); err != nil {
	...
	if err != nil {
		_ = sess.Close()
		t.Fatalf("dial upload (connect_stream leg): %v", err)
	}
	upBytes, _, upErr := masque.MeasureNativeUploadMbps(upConn, bidiCleanupUploadDur)
	_ = upConn.Close()
	if upErr != nil && upBytes == 0 {
		_ = sess.Close()
		t.Fatalf("hybrid upload: %v", upErr)
	}
	masque.WaitNativeConnectIPEgressSettled(ctx, bidiCleanupSettlePause)
	...
	if err != nil {
		_ = sess.Close()
		t.Fatalf("dial download (connect_stream leg): %v", err)
	}
	downBytes, _, downErr := MeasureHybridSmokeDownloadWriteToMbps(downConn, bidiCleanupDownloadDur)
	_ = downConn.Close()
	if downErr != nil && downBytes == 0 {
		_ = sess.Close()
		t.Fatalf("hybrid download WriteTo: %v", downErr)
	}
	if downBytes < bidiCleanupMinBytes {
		_ = sess.Close()
		t.Fatalf("hybrid download after upload=%d bytes want >= %d", downBytes, bidiCleanupMinBytes)
	}
	masque.WaitNativeConnectIPEgressSettled(ctx, bidiCleanupSettlePause)
