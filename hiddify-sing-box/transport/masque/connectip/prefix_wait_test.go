...
type stubPrefixSource struct {
	current []netip.Prefix
	seq     [][]netip.Prefix
	idx     int
}

func (s *stubPrefixSource) CurrentAssignedPrefixes() []netip.Prefix {
	cp := make([]netip.Prefix, len(s.current))
	copy(cp, s.current)
	return cp
}

func (s *stubPrefixSource) LocalPrefixes(ctx context.Context) ([]netip.Prefix, error) {
	if s.idx < len(s.seq) {
		p := s.seq[s.idx]
		s.idx++
