...
type Netstack struct {
	...
	outboundDrainPending  atomic.Bool
	pumpLoopActive        atomic.Bool
...
func NewNetstack(_ context.Context, session PacketSession, opts NetstackOptions) (*Netstack, error) {
	if session == nil {
		return nil, joinStackInit(errors.New("connect-ip packet session is nil"))
	}
	if opts.MTU <= 0 {
		opts.MTU = 1500
	}
...
func (s *Netstack) DialContext(ctx context.Context, destination M.Socksaddr) (net.Conn, error) {
	if err, ok := s.terminalErr.Load().(error); ok && err != nil {
		return nil, joinDial(err)
	}
	if s.closed.Load() {
		return nil, joinDialClosed()
	}
	...
	if !destination.Addr.IsValid() || destination.Port == 0 {
		return nil, joinDialRequiresIP(errors.New("connect-ip tcp dial requires IP destination"))
	}
