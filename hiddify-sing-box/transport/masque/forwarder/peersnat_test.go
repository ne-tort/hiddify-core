...
func TestRewriteOutgoingPeerDstIPv4(t *testing.T) {
	t.Parallel()
	...
	out := RewriteOutgoingPeerDst(pkt, peerPrefixes)
	if got := netip.AddrFrom4([4]byte(out[16:20])); got != peer {
		t.Fatalf("dst after rewrite: got %s want %s", got, peer)
	}
}

func TestRewriteOutgoingPeerDstIPv6WhenIPv4ListedFirst(t *testing.T) {
	t.Parallel()
