...
// RewriteOutgoingPeerDst rewrites IPv4/IPv6 destination to the assigned peer address when
// router-originated replies carry an Internet dst (connect-ip-go policy would drop them).
func RewriteOutgoingPeerDst(packet []byte, peerPrefixes []netip.Prefix) []byte {
	...
	if prefixContainsAddr(peerPrefixes, packetDestinationAddr(packet)) {
		return packet
	}
	peer, ok := peerHostAddrForVersion(peerPrefixes, v)
	if !ok {
		return packet
	}
	switch v {
...
func peerHostAddrForVersion(prefixes []netip.Prefix, version uint8) (netip.Addr, bool) {
	want6 := version == 6
