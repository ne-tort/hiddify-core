...
func NewNATDevice(ctx context.Context, logger logger.ContextLogger, upstream tunAdapter) NatDevice {
...
func (d *natDeviceWrapper) Read(bufs [][]byte, sizes []int, offset int) (n int, err error) {
	...
	return d.tunAdapter.Read(bufs, sizes, offset)
}
...
func (d *natDeviceWrapper) Write(bufs [][]byte, offset int) (int, error) {
	...
	if len(d.buffer) > 0 {
		_, err := d.tunAdapter.Write(d.buffer, offset)
		if err != nil {
			return 0, err
		}
		d.buffer = d.buffer[:0]
