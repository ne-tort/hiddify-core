...
type QueueOutboundElement struct {
	...
	// packet is always a slice of "buffer". The starting offset in buffer is either:
	//  a) MessageEncapsulatingTransportSize+MessageTransportHeaderSize (plaintext from TUN)
	//  b) 0 (post-encryption, full UDP payload including encapsulating prefix)
	packet  []byte
...
func (peer *Peer) SendHandshakeResponse() error {
	...
	err = peer.sendBuffersPlain([][]byte{packet})
	if err != nil {
...
func (device *Device) SendHandshakeCookie(initiatingElem *QueueHandshakeElement) error {
	...
	if padding := device.paddings.cookie; padding > 0 {
		...
		copy(buf[padding:], packet)
		packet = buf
		// Cookie reply bytes start after random prefix; offset selects UDP payload slice for Send.
		return device.net.bind.Send([][]byte{packet}, initiatingElem.endpoint, padding)
	}

	full := make([]byte, MessageEncapsulatingTransportSize+len(packet))
	copy(full[MessageEncapsulatingTransportSize:], packet)
	return device.net.bind.Send([][]byte{full}, initiatingElem.endpoint, MessageEncapsulatingTransportSize)
}
