...
func genTestPair(
	...
	if realSocket {
		binds[0], binds[1] = conn.NewDefaultBind(nil), conn.NewDefaultBind(nil)
	} else {
...
func (b *fakeBindSized) SendWithoutModify(bufs [][]byte, ep conn.Endpoint, offset int) error {
	return nil
}

func (b *fakeBindSized) SetReservedForEndpoint(netip.AddrPort, [3]byte) {}
