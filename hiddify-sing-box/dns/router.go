...
func NewRouter(ctx context.Context, logFactory log.Factory, options option.DNSOptions) *Router {
	...
	router.reach = newAdaptiveReachability(router.logger, router.outbound)
	return router
}

func (r *Router) materializeAdaptiveStrategy(strategy C.DomainStrategy) C.DomainStrategy {
	if strategy != C.DomainStrategyAdaptive {
		return strategy
	}
	if r.reach == nil {
		return C.DomainStrategyAsIS
	}
	return r.reach.effectiveStrategy()
}
...
func (r *Router) Exchange(ctx context.Context, message *mDNS.Msg, options adapter.DNSQueryOptions) (*mDNS.Msg, error) {
	...
	if options.Transport != nil {
		...
		options.Strategy = r.materializeAdaptiveStrategy(options.Strategy)
		...
		for {
			...
			dnsOptions.Strategy = r.materializeAdaptiveStrategy(dnsOptions.Strategy)
