...
// parseWGPrefix accepts CIDR (10.0.0.1/24) or a bare address (10.0.0.1 -> /32, fe80::1 -> /128), as in many .conf exports.
...
func AWGSingboxTxt(content string) (*T.Endpoint, error) {
		...
		case "peer":
			...
			case "AllowedIPs":
				...
				for _, a := range strings.Split(val, ",") {
					pfx, err := parseWGPrefix(a)
					if err != nil {
						return nil, fmt.Errorf("invalid AllowedIPs: %w", err)
					}
					allowed = append(allowed, pfx)
				}
				peer.AllowedIPs = badoption.Listable[netip.Prefix](allowed)
	...
	hasAwgObfs := jc+jmin+jmax+s1+s2+s3+s4 != 0 || h1+h2+h3+h4+i1+i2+i3+i4+i5 != ""
	if !hasAwgObfs {
		wgOpt := &T.WireGuardEndpointOptions{
...
func AWGSingbox(raw string) (*T.Endpoint, error) {
	...
	var out *T.Endpoint
	isAwg := opts.Jc+opts.Jmin+opts.Jmax+opts.S1+opts.S2+opts.S3+opts.S4 != 0 || opts.H1+opts.H2+opts.H3+opts.H4+opts.I1+opts.I2+opts.I3+opts.I4+opts.I5 != ""
